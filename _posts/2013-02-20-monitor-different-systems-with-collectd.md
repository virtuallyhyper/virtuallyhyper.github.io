...
author: karim
comments: true
date: 2013-02-20 20:10:46+00:00
layout: post
slug: monitor-different-systems-with-collectd
...
Lastly I wanted to compare it to a large application just to see it's Pro's and Con's. Since Nagios and Cacti were out of the picture, I decided to try **Zenoss**. There are a couple of sites that talk about the differences between Nagios and Zenoss, here are a few:
  ...
  * [Zennos VS. Nagios](http://community.zenoss.org/docs/DOC-5858)
...
Now let's just copy the whole directory to a folder where your web Server (I was using _Apache_ for other uses) can host the files:
...
Lastly make sure _collectd_ looks under the correct location (where the _RRD_ files are stored):
...
[![collectd-web-first-page](http://virtuallyhyper.com/wp-content/uploads/2013/02/collectd-web-first-page.png)](http://virtuallyhyper.com/wp-content/uploads/2013/02/collectd-web-first-page.png)
...
[![collectd-web-freebsd-cpu](http://virtuallyhyper.com/wp-content/uploads/2013/02/collectd-web-freebsd-cpu.png)](http://virtuallyhyper.com/wp-content/uploads/2013/02/collectd-web-freebsd-cpu.png)
...
we have two disks that are online. Collectd has a plugin called **exec**, it allows you to run a command and plot values from the results of that command. More information can be seen [here](http://collectd.org/documentation/manpages/collectd-exec.5.shtml) and [here](https://collectd.org/wiki/index.php/Plugin:Exec). If we had SMART capabilities, we could use the scripts created from [this](http://devel.dob.sk/collectd-scripts/) post. But I will settle with just plotting how many disks are currently online. So I wrote this script:
...
[![collectd-web-freebsd-raid-gauge](http://virtuallyhyper.com/wp-content/uploads/2013/02/collectd-web-freebsd-raid-gauge.png)](http://virtuallyhyper.com/wp-content/uploads/2013/02/collectd-web-freebsd-raid-gauge.png)
