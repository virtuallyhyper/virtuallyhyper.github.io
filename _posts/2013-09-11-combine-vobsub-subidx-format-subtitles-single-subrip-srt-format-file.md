...
author: karim
comments: true
date: 2013-09-11 21:18:24+00:00
layout: post
slug: combine-vobsub-subidx-format-subtitles-single-subrip-srt-format-file
title: Combine VobSub (sub/idx) Format Subtitles into a Single SubRip (srt) Format
  File
wordpress_id: 9445
categories:
- OS
...
I recently had a situation where I had 2 _Avi_ files and 2 _Sub/Idx_ files, and I just wanted to combine them into one _Avi_ file and one _srt_ file. Here are the files that I started out with:
...
[![different_format_of_subtitles](http://virtuallyhyper.com/wp-content/uploads/2013/09/different_format_of_subtitles.png)](http://virtuallyhyper.com/wp-content/uploads/2013/09/different_format_of_subtitles.png)
...
After that you should have one big _Avi_ file.
...
Since we have more than two _texts_ that means we have multiple languages in the **VobSub** files. Notice the ID fields: 0x24 and 0x25. We will actually use those later on. You can also use **tcscan** utility to check for the subtitle IDs as well:
...
[![feh_after_subtittle2pgm](http://virtuallyhyper.com/wp-content/uploads/2013/09/feh_after_subtittle2pgm.png)](http://virtuallyhyper.com/wp-content/uploads/2013/09/feh_after_subtittle2pgm.png)
...
[![aspell_fix_spelling](http://virtuallyhyper.com/wp-content/uploads/2013/09/aspell_fix_spelling.png)](http://virtuallyhyper.com/wp-content/uploads/2013/09/aspell_fix_spelling.png)
...
the **-s** specifies the subtitle ID (so I am picking the first english subtitles). The first _file1_ specifies the basename for the **.sub** and **.idx** files. So you need to have _file1.sub_ and _file1.idx_ in the directory where you run that command from. After that it's done you will have the 788 **pgm** images and an **xml** file (similar to the **.srtx** file from **subtitle2pgm** command) for the description file:
...
[![feh_after_subp2pgm](http://virtuallyhyper.com/wp-content/uploads/2013/09/feh_after_subp2pgm.png)](http://virtuallyhyper.com/wp-content/uploads/2013/09/feh_after_subp2pgm.png)
...
There is another tool that was recently released that does all the above in one swoop. It wasn't part of the _yum_ repository so I compiled it manually.
...
Later on, I got a good version of the **SRT** file and I wanted to compare the times from caption _788_ to _789_. So I ran the following:
