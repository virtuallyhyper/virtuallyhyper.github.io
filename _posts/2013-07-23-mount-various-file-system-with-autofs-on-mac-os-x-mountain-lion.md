...
author: karim
comments: true
date: 2013-07-23 14:54:45+00:00
layout: post
slug: mount-various-file-system-with-autofs-on-mac-os-x-mountain-lion
...
Go to [Installing MacPorts](http://www.macports.org/install.php) and download the _pkg_ installer. Launch installer and you will see the following:





[![mac_ports_install](http://virtuallyhyper.com/wp-content/uploads/2013/07/mac_ports_install.png)](http://virtuallyhyper.com/wp-content/uploads/2013/07/mac_ports_install.png)





Follow the on-screen instructions install _Mac Ports_.
...
_X-Code_ is necessary for Mac Ports, so go ahead and install it from [XCode](https://developer.apple.com/xcode/). It will actually start up the _App Store_ to do the download. After it's done your _App Store_ will look like this:





[![xcode-install-app-store](http://virtuallyhyper.com/wp-content/uploads/2013/07/xcode-install-app-store.png)](http://virtuallyhyper.com/wp-content/uploads/2013/07/xcode-install-app-store.png)
...
[![xcode-preferences](http://virtuallyhyper.com/wp-content/uploads/2013/07/xcode-preferences.png)](http://virtuallyhyper.com/wp-content/uploads/2013/07/xcode-preferences.png)
...
[![xcode-command-line-tools](http://virtuallyhyper.com/wp-content/uploads/2013/07/xcode-command-line-tools.png)](http://virtuallyhyper.com/wp-content/uploads/2013/07/xcode-command-line-tools.png)
...
Mac Ports are like any package manager, it allows you to search for packages and install them. For example here is what I did to search for _sshfs_:
...
So if I wanted to enable _debug_ binaries for the **mysql56** package I would run the following:
...
Anyways, to install _sshfs_, run the following:
...
You can un-mount the _sshfs_ mount point like this:
...
The way that _autofs_ works is the type of the file system is always prepended with the _mount__ prefix and that is run to actually mount that file system. So if you specify **smbfs**, there must be a **mount_smbfs** command. Here are the available **mount** commands on the system:
...
So we need to create a _mount_sshfs_ command. We can just create a _symlink_ to the **sshfs** command like so:
