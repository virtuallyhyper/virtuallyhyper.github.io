...
author: karim
comments: true
date: 2012-10-22 13:23:45+00:00
layout: post
slug: setup-fedora-17-with-nvidia-geforce-6200-to-connect-to-a-tv-and-function-as-an-xbmc-media-center
title: Setup Fedora 17 with nVidia GeForce 6200 Video Card to Connect to a TV and
  Function as an XBMC Media Center
wordpress_id: 4338
categories:
- Home Lab
- OS
...
I looked over the _/usr/share/doc/initscripts-9.37.1/sysconfig.txt_ file , and I saw the following:
...
So I setup up my _/etc/sysconfig/desktop_ file to look like this:
...
I rebooted my machine, but when X started the display manager just kept scrolling up and down like the refresh rate was off or something. I then ran across this article "[nVidia/TV-OUT](http://en.wikibooks.org/wiki/NVidia/TV-OUT)" and it had a really good step by step process on how to generate a proper _xorg.conf_ file for a tv-out setup. After following the instructions, I had created the following _xorg.conf_ file:
...
The _akmod_ option sounded perfect for my setup. I also checked out the RPM Fusion page on [Akmods](http://rpmfusion.org/Packaging/KernelModules/Akmods), from the page:
...
I decided to try out the _akmod_ install.
...
The last command created the following _xorg.conf_ file:
...
After I ran that I also noticed that the _nouveau_ module was actually blacklisted by the _rpmfusion_ install:
...
One more reboot and my GDM finally started up without refresh issues.I then tried to login, but my keyboard wasn't working. I checked out _/var/log/Xorg.0.log_ file and I saw the following:
...
After that, I restarted and the keyboard started to work with Xorg just fine. I also wanted to ensure that the _nouveau_ wasn't loaded:
...
I edited my _/etc/default/grub_ file and changed the following lines to look like this:
...
I already had the top one, so I went ahead and installed the _terminus_ fonts. After I installed the font I checked out the README file and I saw the following:
...
But it still gave me the same error. I then ran across [this](http://ubuntuforums.org/showthread.php?t=1667602) Ubuntu forum. They fixed the issue by changing the console resolution via the _vga=792_ option in their grub configuration. I wanted to find out what setting I should use . After some googling around I ran into this article "[Change tty font size with Grub 2 boot console resolution](http://forum.linuxcareer.com/threads/1661-Change-tty-font-size-with-Grub-2-boot-console-resolution)". From the article:
...
[![](http://virtuallyhyper.com/wp-content/uploads/2012/10/console_resolution_tab.png)](http://virtuallyhyper.com/wp-content/uploads/2012/10/console_resolution_tab.png)





So I made the following change to my _/etc/default/grub_ file:
...
and then I regenerated my _grub.cfg_ file:
...
That gave me a list of all the available resolutions. I decided to set it to 800x600 since that is what the message showed :). I rebooted and then edited my _/etc/default/grub_ file to have the following:
...
After I rebooted the font looked awesome. To change the font permanently you can edit your _/etc/sysconfig/i18n_ file and make the following change:
...
From the man page of _setsysfont_:
...
So _setsysfont_ will read the defined font from _/etc/sysconfig/i18n_ and apply that font to the current terminal.





The last thing I wanted to do is setup XBMC to login automatically. After I installed GDM, I noticed that one of the drop down options for a window manager was XBMC. So all I had to do was run _gdmsetup_ and set my user to auto login to that session. I was surprised to learn that _gdmsetup_ was deprecated and the replacement utility is currently under development. For more information check out [this](https://bugzilla.redhat.com/show_bug.cgi?id=439881) RHEL bug. From the bugzilla page:
...
After looking over "[Custom GDM Configuration For Auto And Timed Login](https://wiki.archlinux.org/index.php/GDM#GDM_Will_Not_Load_After_Attempting_to_Set-Up_Automatic_Login)" and [Gnome Display Manager Reference Manual](https://help.gnome.org/admin/gdm/stable/), I ended up adding the following:
...
to the _/etc/gdm/custom.conf_ file. You can get a list of available desktop sessions by doing the following:
...
If for whatever reason, you need to have a second non-XBMC X-session, you can hit CTRL-ALT-F3, login as a regular user, and define a window manager you want to use by default. This is done by creating a _.xinitrc_ file and specifying the window manager of your choice. Here is how mine looked like:
...
[![](http://virtuallyhyper.com/wp-content/uploads/2012/10/xbmc_remote_control.png)](http://virtuallyhyper.com/wp-content/uploads/2012/10/xbmc_remote_control.png)
...
I then saved the _iptables_ settings.
