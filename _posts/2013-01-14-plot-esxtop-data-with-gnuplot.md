...
author: karim
comments: true
date: 2013-01-14 18:16:58+00:00
layout: post
slug: plot-esxtop-data-with-gnuplot
...
[elatov@klaptop]$ awk -F , '{print $1","$16391}' data > nfs_read_latency_data
...
[elatov@klaptop]$ head nfs_read_latency_data 
...
[elatov@klaptop]$ gnuplot -p -e 'set grid;set xdata time; set timefmt "%m/%d/%Y %H:%M:%S"; set datafile sep ","; set y2tics autofreq; set format x "%H:%M" ; plot "nfs_read_latency_data" using 1:2 with lines title "Read Lat/s'
...
[![gnuplot_graph_nfs_latency](http://virtuallyhyper.com/wp-content/uploads/2012/12/gnuplot_graph_nfs_latency.png)](http://virtuallyhyper.com/2013/01/plot-esxtop-data-with-gnuplot/gnuplot_graph_nfs_latency/)
...
[elatov@klaptop]$ grep '20:20:' nfs_read_latency_data > nfs_read_latency_data_20_20
...
[elatov@klaptop]$ cat nfs_read_latency_data_20_20 
...
[elatov@klaptop]$ gnuplot -p -e 'set grid;set xdata time; set timefmt "%m/%d/%Y %H:%M:%S"; set datafile sep ","; set y2tics autofreq; set format x ":%M:%S" ; plot "nfs_read_latency_data_20_20" using 1:2 with lines title "Read Lat/ms'
...
[![gnuplot_nfs_latency_20:20](http://virtuallyhyper.com/wp-content/uploads/2012/12/gnuplot_nfs_latency_2020.png)](http://virtuallyhyper.com/2013/01/plot-esxtop-data-with-gnuplot/gnuplot_nfs_latency_2020/)
...
[elatov@klaptop]$ grep '20:20:' data | awk -F , '{print$1","$16386","$16391}' > nfs_latency_and_nfs_reads_data_20_20
...
[elatov@klaptop]$ head nfs_latency_and_nfs_reads_data_20_20
...
[elatov@klaptop]$ gnuplot -p -e 'set grid;set xdata time; set timefmt "%m/%d/%Y %H:%M:%S"; set datafile sep ","; set y2tics autofreq; set format x ":%M:%S" ; plot "nfs_latency_and_nfs_reads_data_20_20" using 1:3 with lines title "Read Lat/ms"; replot "nfs_latency_and_nfs_reads_data_20_20" using 1:2 with lines title "Read/s'
...
[![gnuplot_nfs_lat_nfs_reads](http://virtuallyhyper.com/wp-content/uploads/2012/12/gnuplot_nfs_lat_nfs_reads.png)](http://virtuallyhyper.com/2013/01/plot-esxtop-data-with-gnuplot/gnuplot_nfs_lat_nfs_reads/)
...
[![gnuplot_time_formate](http://virtuallyhyper.com/wp-content/uploads/2012/12/gnuplot_time_formate.png)](http://virtuallyhyper.com/2013/01/plot-esxtop-data-with-gnuplot/gnuplot_time_formate/)
...
> 
The command _set datafile separator "char"_ tells gnuplot that data fields in subsequent input files are separated by 'char' rather than by whitespace. The most common use is to read in csv (comma-separated value) files written by spreadsheet or database programs.
...
[![gnuplot_time_format_tics](http://virtuallyhyper.com/wp-content/uploads/2012/12/gnuplot_time_format_tics.png)](http://virtuallyhyper.com/2013/01/plot-esxtop-data-with-gnuplot/gnuplot_time_format_tics/)
...
Each 'entry' may be a simple column number that selects the value from one field of the input t, an expression enclosed in parentheses, or empty.
...
Each plot style has its own expected set of data entries in a data file. For example by default the lines style expects either a single column of y values (with implicit x ordering) or a pair of columns with x in the first and y in the second.
