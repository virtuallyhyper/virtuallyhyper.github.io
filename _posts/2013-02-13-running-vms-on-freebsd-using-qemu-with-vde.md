...
author: karim
comments: true
date: 2013-02-13 17:34:00+00:00
layout: post
slug: running-vms-on-freebsd-using-qemu-with-vde
...
I decided to run some VMs on my FreeBSD server. Checking over [this](http://en.wikipedia.org/wiki/Comparison_of_platform_virtual_machines) wikipedia page, it seems that _KVM_, _QUME_, and _VirtualBox_ were my only options. I remember using _VirtualBox_ with FreeBSD before and even though it's really solid, the compile would take a while. I wanted to try out _KVM_ but it seems the project, which was started in 2007, never really took off. You can try to compile the _KVM_ module but I didn't want to try something that hasn't been developed since 2007. There is actually a nice conversation about that [here](http://lists.freebsd.org/pipermail/freebsd-virtualization/2011-July/000730.html). In that same article they mention a new hypervisor that is specifically made for FreeBSD. The project is called [BHyve](http://bhyve.org/), apparently it's being developed in collaboration with _NetAPP_. The project is currently supported in FreeBSD 10 and I was running FreeBSD 9. So after all of my research, I decided to give _Qemu_/_KQemu_ a try.





Now what exactly is the difference between _Qemu_ and _KVM_? from the _KVM_ [FAQ](http://www.linux-kvm.org/page/FAQ#What_is_the_difference_between_KVM_and_QEMU.3F):
...
And what about _KQemu_? from the [Qemu](http://en.wikipedia.org/wiki/QEMU) wikipedia page:
...
So to re-phrase my initial sentence: I will be virtualizing/emulating an OS within the FreeBSD kernel and using the _KQemu_ acceleration kernel module to improve performance/execution.





Now let's get started with the setup, first let's install _Qemu_, instructions can be found [here](http://www.linux-kvm.org/page/BSD) and [here](https://wiki.freebsd.org/qemu):
...
After the compile finished you can enable the _KQemu_ module to be loaded on boot, this is done by adding the following to the **/etc/rc.conf** file:
...
This will load the _KQemu_ module on start up and it will also check if the _aio_ module is loaded. If the _aio_ module is not loaded, it will also load it.
...
That looks good. As a quick test, let's create an _img_ file:
...
The '**-f**' option specifies the format of the image, from the _man_ page here are the options:
...
Since I will be running RHEL on the VM (that will use an _ext_ filesystem), I decided to use the _raw_ format. Now to start the VM using that _img_ file as the hard drive:
...
Now to test connectivity, from a Fedora machine I installed a _VNC_ client:
...
A _VNC_ window popped up without any issues, and I saw the RHEL CD boot up. Now if we want our VM to have access to the network then we have a couple of options. From the [Qemu Networking](http://wiki.qemu.org/Documentation/Networking) page, we can use "**User Mode Networking**":
...
[![qemu_user_networking](http://virtuallyhyper.com/wp-content/uploads/2013/02/qemu_user_networking.png)](http://virtuallyhyper.com/wp-content/uploads/2013/02/qemu_user_networking.png)
...
Since Jarret already did a [post](http://virtuallyhyper.com/2012/07/installing-kvm-as-a-virtual-machine-on-esxi5-with-bridged-networking/) on the bridged/_tap_ networking with _KVM_, I decided to try out the _VDE_ setup. First let's install the _VDE_ Package:
...
After that is installed we should create a _tap_ interface to act as an uplink for our _VDE_-switch. Instructions for this can be found [here](http://bsdwiki.reedmedia.net/wiki/networking_qemu_virtual_bsd_systems.html) and [here](https://wiki.freebsd.org/qemu). First let's create the _bridge_:
...
Then let's create the _tap_ interface:
...
Now let's bridge our physical interface with our _tap_ interface:
...
Since we will be connecting to the _tap_ device with regular users, let's allow regular users to connect to our _tap_ device:
...
That looks good. Now let's create a _VDE_-Switch and add **tap0** as our uplink:
...
Most of the information regarding the _VDE_ was taken from [here](http://wiki.v2.cs.unibo.it/wiki/index.php?title=VDE#Some_usage_examples). BTW, there is also a web and telnet client for the _VDE_-switch. Instructions on how to set that up are [here](http://wiki.v2.cs.unibo.it/wiki/index.php?title=Vdetelweb:_Telnet_and_Web_management_for_VDE); '**unixterm**' was good enough for me :)





Now starting up the VM and connecting it to our _VDE_-Switch:
...
Since we don't want to keep recreating the above settings, let's go ahead and setup all the above options/settings to be auto configured on boot. We already enabled the _kqemu_ module to be loaded on boot, now let's setup the bridge and _tap_ interfaces to be created on boot. Add the following to the **/etc/rc.conf** file:
...
Now let's enable the _sysctl_ options. Add the following to the **/etc/sysctl.conf** file:
...
Lastly, create the _VDE_-Switch boot. Add the following to the **/etc/rc.local** file:
...
So I rebooted the FreeBSD host and I wanted to make sure all the settings look good. First I wanted to make sure my **bridge0** and **tap0** interfaces were setup: 
    ...
    <code>elatov@freebsd:~> ifconfig tap0 
...
That looked good. Next I wanted to make sure my _VDE_-Switch was created:
...
and I wanted to make sure the _sysctl_ settings looked good as well:
...
Everything looked good. As Jarret mentioned in his [post](http://virtuallyhyper.com/2012/07/installing-kvm-as-a-virtual-machine-on-esxi5-with-bridged-networking/), there are a lot of ways to go about managing VMs. Here is [web-page](http://www.linux-kvm.org/page/Management_Tools) with all the different tools. Checking over the FreeBSD ports, I only found the following:
...
All of the above (except **aqemu**) depend on the **libvirt** libraries. After further investigation it turned out that _libvirt_ and _VDE_ don't work together... yet. From [here](https://gist.github.com/1787749), a snippet:
