...
author: jarret
comments: true
date: 2012-04-09 17:49:56+00:00
layout: post
slug: vmware-snapshot-troubleshooting
...
For a long time I was consistently looking into [VMware snapshot](http://kb.vmware.com/kb/1015180) issues. I developed a small course to teach basic snapshot troubleshooting snapshots along with some more advanced snapshot troubleshooting techniques. The course has been converted to this article.

The basics about snapshot are discussed in this [VMware KB](http://kb.vmware.com/kb/1025279). In this article we will touch base on the basic snapshot operations and data safe operations.

The first thing to understand is how to properly [consolidate snapshots correctly](http://kb.vmware.com/kb/1007849). If this operation fails we have to investigate the different types of errors and then troubleshoot further. Before we touch on this let's take a look at the basic snapshot operations.
...
We generally see snapshots taken through the API because of backup products, but they can also be taken through the vSphere Client as well. Page 145 of the [Virtual Machine Administration Guide](http://www.vmware.com/pdf/vsphere4/r41/vsp_41_vm_admin_guide.pdf) describes the process of taking a snapshot.
...
More information on Quiesced snapshots can be found in [this KB](http://kb.vmware.com/kb/1015180). If you are having trouble taking a quiesced snapshot please see [this KB](http://kb.vmware.com/kb/1007696), or [this KB if you are using Windows 2008 (R2)](http://kb.vmware.com/kb/1031298).
...
Reverting will restore the state of the VM when the snapshot was taken. It is also known as Restoring a snapshot. Reverting to a snapshot will **destroy all data** since the snapshot was taken so make sure to have backups before doing this operation. Page 148 of the [Virtual Machine Administration Guide](http://www.vmware.com/pdf/vsphere4/r41/vsp_41_vm_admin_guide.pdf) describes the process of reverting to a snapshot.
...
Page 34 of the [Virtual Machine Administration Guide](http://www.vmware.com/pdf/vsphere4/r41/vsp_41_vm_admin_guide.pdf) describes the process of cloning the VM.
...
There are many problems that one can run into with snapshot problems. The issues below are advanced and can be fixed with a little work. I will do a write up for each one and update this list to link to each article.
  ...
  * [Non-Linear Snapshot Chain](http://virtuallyhyper.com/index.php/2012/04/advanced-snapshot-troubleshooting-non-linear-vmware-snapshot-chain/)

	
  * [Invalid parentCID/CID chain](http://virtuallyhyper.com/2012/04/advanced-snapshot-troubleshooting-invalid-parentcidcid-chain/)

	
  * [Incorrect VMDK Geometry](http://virtuallyhyper.com/2012/04/advanced-snapshot-troubleshooting-incorrect-vmdk-geometry/)
