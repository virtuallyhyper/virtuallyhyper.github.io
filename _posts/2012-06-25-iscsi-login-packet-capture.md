...
author: karim
comments: true
date: 2012-06-25 16:25:50+00:00
layout: post
slug: iscsi-login-packet-capture
...
What is seen in a packet capture when an ESX iSCSI initiator success fully logs into the iSCSI target? Since iSCSI uses TCP, the first thing that we are going to see is a _3-way TCP hand shake_ (More information on 3-way TCP handshake can be found [here](http://www.inetdaemon.com/tutorials/internet/tcp/3-way_handshake.shtml)). Now let's follow the packets:
...
So we are getting a Login command with the code of _0x03_. From the iSCSI RFC:
...
So we sent a _Login Request_, from the RFC:
...
Also we see that we have the _T_ flag set:
...
So we sent a _Login Request_ and we are ready to send more data, now looking at the next packet:
...
We can see that all the key/value pairs were negotiated successfully and this was a _Login Response with Login Accept as a final response_. Next we see a _Text Command_:
...
The _Text Command_ is _0x04_ and from the above snippet of the RFC we can see that it's a _Text Request_:
...
So now that we have connected to the target we are going to ask some information and that is what a _Text command_ does, here is what we see in the next packet:
...
For our _Text Request_ we have sent a _SendTargets=All_:
...
Since I am using a discovery session (In ESX "Dynamic Discovery"), this makes perfect sense. Next we get a _Text Response_
...
We can see that in the response we received the _TargetName_ and _TargetAddress_ of what our iSCSI initiator has access to, and this is also expected; per the RFC:
...
We got our targets, and now we actually send a _Logout Request_:
...
Why did we get a _Logout Request_? since this was the first login this makes sense:
...
After the _Logout Request_ we see a _Logout Response_:
...
Since we are actually not done, we keep the connection alive by using the _NOP-out_ and _NOP-in_ packets:
...
We then see the _Login Request_ with the login parameters:
...
[elatov@machine ~]$ tshark -r iscsi_login.pcap -V -O data frame.number==28
...
Then we get the _Login Response_:
...
After that the initiator sends _Text Request_ to get all the available targets with a _SendTargets=All_ command:
...
Then we get a _Text Response_ from the _Text Request_ with the available targets:
...
[![](http://virtuallyhyper.com/wp-content/uploads/2012/04/iscsi_1.png)](http://virtuallyhyper.com/wp-content/uploads/2012/04/iscsi_1.png)

[![](http://virtuallyhyper.com/wp-content/uploads/2012/04/iscsi_2.png)](http://virtuallyhyper.com/wp-content/uploads/2012/04/iscsi_2.png)
